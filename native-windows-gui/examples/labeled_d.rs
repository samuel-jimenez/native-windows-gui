/*!
    A very simple application that show how to use labeled controls.

    Requires the following features: `cargo run --example labeled_d --features "labeled"`
*/

extern crate native_windows_derive as nwd;
extern crate native_windows_gui as nwg;

use nwd::NwgUi;
use nwg::{
    NativeUi,
    taffy::{
        LengthPercentage, LengthPercentageAuto,
        geometry::{Rect, Size},
        style::{AlignSelf, Dimension, FlexDirection},
        style_helpers::{auto, length, percent},
    },
};

// Flexbox style
const FIFTY_PC: Dimension = Dimension::percent(0.5);
const PT_10: LengthPercentage = LengthPercentage::length(10.0);
const PT_5: LengthPercentageAuto = LengthPercentageAuto::length(5.0);
const PADDING: Rect<LengthPercentage> = Rect {
    left: PT_10,
    right: PT_10,
    top: PT_10,
    bottom: PT_10,
};
const MARGIN: Rect<LengthPercentageAuto> = Rect {
    left: PT_5,
    right: PT_5,
    top: PT_5,
    bottom: PT_5,
};

#[derive(Default, NwgUi)]
pub struct FlexBoxApp {
    #[nwg_control(size: (700, 300), position: (300, 300), title: "Flexbox example")]
    #[nwg_events( OnWindowClose: [nwg::stop_thread_dispatch()] )]
    window: nwg::Window,

    #[nwg_layout(parent: window, flex_direction: FlexDirection::Row, padding: PADDING)]
    layout: nwg::FlexboxLayout,

    #[nwg_control(text: "Btn 1")]
    #[nwg_layout_item(layout: layout, margin: MARGIN,
        max_size: Size { width: length(200.0), height: auto() },
        size: Size { width: percent(0.25), height: auto() }
    )]
    button1: nwg::Button,

    #[nwg_control(label: "Edit Control")]
    #[nwg_layout_item(layout: layout, margin: MARGIN,
        max_size: Size { width: length(400.0), height: auto() },
        size: Size { width: FIFTY_PC, height: auto() }
    )]
    edit: nwg::LabeledEdit,

    #[nwg_control(text: "Btn 2")]
    #[nwg_layout_item(layout: layout,
        margin: MARGIN,
        align_self: Some(AlignSelf::FlexEnd),
        size: Size { width: percent(0.25), height: FIFTY_PC }
    )]
    button2: nwg::Button,

    #[nwg_control(text: "Btn 3")]
    #[nwg_layout_item(layout: layout,
        margin: MARGIN,
        flex_grow: 2.0,
        size: Size { width: auto(), height: auto() }
    )]
    button3: nwg::Button,
}

fn main() {
    nwg::init().expect("Failed to init Native Windows GUI");
    nwg::Font::set_global_family("Segoe UI").expect("Failed to set default font");

    let _ui = FlexBoxApp::build_ui(Default::default()).expect("Failed to build UI");

    nwg::dispatch_thread_events();
}
